...
ui <- dashboardPage(skin="black",
                    dashboardHeader(title=tags$em("Step II XGBoost-based prediction for diagnosis of biliary atresia", style="text-align:center;color:#0063B7;font-size:100%"),titleWidth = 800),
                                  ...
                                  column(width = 12,
                                    ...
                                    clicking the button below. This dataset came from the Seoul Asan Medical Center (n=1605) and Seoul National University Hostpital (n=912), Korea.", 
                                         ...
                                         Hb; haemoglobin (g/dL), PTINR; prothrombin time (international normalized ratio), AST; aspartate aminotransferase (IU/L), 
                                                 ...
                                                 US; presence of any signs of biliary atresia (1=having any signs), and HBS; any signs of hepatybiliary scan (1=not excreted or severely delayed). US and HBS were not included to Step II prediction", 
                                                 style="font-size:100%"),
                                         br()
                                  )),
                                fluidRow(
  ...
  predictions<-reactive({
    ...
    if (is.null(inFile)){
      ...
      withProgress(message = 'Predictions in progress. Please wait ...', {
        input_data =  readr::read_csv(input$file1$datapath, col_names = TRUE)
        test.X <- as.matrix(input_data[1:1,2:17])
